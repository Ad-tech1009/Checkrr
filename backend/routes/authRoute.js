...
router.post('/signup', async (req, res) => {
  try {
      ...
      await newUser.save();
      const authToken = jwt.sign({ userId: newUser._id }, process.env.JWT_SECRET_KEY, { expiresIn: '10m' });
      const refreshToken = jwt.sign({ userId: newUser._id }, process.env.JWT_REFRESH_SECRET_KEY, { expiresIn: '30m' });
...
router.post('/login', async (req, res) => {
  ...
  if (!isMatch) {
      console.log('password not matched', user.password, password);
      return res.status(400).json(createResponse(false, 'Invalid credentials'));
  }
